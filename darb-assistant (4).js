(() => {
  ...
  const MODEL = 'gemini-2.5-flash';          // swap to your current model if needed
  ...
  const PRESETS = [
    ...
    { label: 'Tighten',    instr: 'Rewrite to be concise and clear without losing any factual content. Return only the rewritten text.' },
    ...
    { label: 'Holdings',   instr: 'The text is a list of crypto holdings, often with each asset duplicated (name, name with ticker, then weight). Output one line per asset as "Name (TICKER) - weight%". Do NOT fabricate or estimate any missing weight or ticker; if a value is absent, write "[missing]". Return only the list.' },
    { label: 'Holdings CSV', instr: 'The input is a list of crypto holdings. Output CSV with header "Asset,Ticker,Weight", one row per asset. Do NOT fabricate missing values; leave the cell blank and never guess. Return only the CSV.' },
    { label: 'Read image',  instr: 'Read the attached image of a holdings breakdown. Output CSV with header "Asset,Ticker,Weight". Include a row only where the name/ticker/weight is printed as legible text. Where a box shows only an icon and a percentage with no text label, write "UNVERIFIED" for Asset and Ticker but still record the Weight. Do NOT identify assets from logos or icons. Return only the CSV.' },
  ];
  ...
  function renderAssistant(panel) {
    panel.innerHTML = `
      <div class="darb-ai-head">
        ...
        <span class="tools"><span class="darb-ai-settings" title="Change or remove key">&#9881;</span><span class="darb-ai-x" title="Close">&times;</span></span>
      </div>
